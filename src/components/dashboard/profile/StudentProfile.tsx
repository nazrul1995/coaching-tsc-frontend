    ...
    return (
      <div className="text-center py-20">
        ...
        <Link href="/dashboard/student-form">
          <Button className="bg-[#adc6ff] text-[#0b1326] hover:bg-[#adc6ff]/90 px-8">
      ...
      <div className="bg-white/5 border border-white/10 rounded-3xl p-8 md:p-10 flex flex-col md:flex-row gap-8 items-center">
        ...
        <div className="relative w-40 h-40 md:w-48 md:h-48 rounded-3xl overflow-hidden border-4 border-[#adc6ff]/30 flex-shrink-0">
          {student.photo ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-[#adc6ff]/20 to-[#6ffbbe]/20 flex items-center justify-center text-7xl font-semibold text-white">
        ...
        <div className="flex-1 text-center md:text-left">
          <div className="flex items-center justify-center md:justify-start gap-3 mb-3">
            ...
            <span className="px-4 py-1 bg-[#adc6ff]/10 text-[#adc6ff] text-sm font-semibold rounded-2xl">
              STUDENT
            </span>
          </div>
          ...
          <div className="mt-8 flex justify-center md:justify-start gap-8 text-sm">
            <div>
              <div className="text-[#6ffbbe] font-semibold">{student.className}</div>
              <div className="text-white/50 text-xs">Class</div>
            </div>

            {student.group && (
              <div>
                <div className="text-[#adc6ff] font-semibold">{student.group}</div>
            ...
            {student.batch && (
              <div>
                <div className="text-white font-semibold">{student.batch}</div>
        ...
        <Link href="/dashboard/profile">
          <Button className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-8 py-6 rounded-3xl transition-all">
            <Edit size={20} />
            Edit Profile
          </Button>
        </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
          <div className="flex items-center gap-3 mb-6">
            <GraduationCap className="text-[#6ffbbe]" size={26} />
            <h3 className="text-2xl font-bold text-white">Academic Information</h3>
          </div>

          <div className="space-y-6">
            <div>
              <p className="text-white/50 text-sm">Class</p>
              <p className="text-white text-xl">{student.className}</p>
            </div>

            {student.group && (
              <div>
                <p className="text-white/50 text-sm">Group</p>
                <p className="text-white text-xl capitalize">{student.group}</p>
              </div>
            )}

            {student.batch && (
              <div>
                <p className="text-white/50 text-sm">Batch</p>
                <p className="text-white text-xl">{student.batch}</p>
              </div>
            )}
            ...
            {student.institution && (
              <div>
                ...
                <p className="text-white text-xl">{student.institution}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8">
          <div className="flex items-center gap-3 mb-6">
            <Users className="text-[#adc6ff]" size={26} />
            <h3 className="text-2xl font-bold text-white">Activity Information</h3>
          </div>

          <div className="space-y-6">
            <div>
              <p className="text-white/50 text-sm">Joined Platform</p>
