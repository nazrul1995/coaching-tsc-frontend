...
export default function ExamResultManagementPage() {
 ...
 if(loading)return <LoadingState label="Loading exam results..."/>;
 return <div className="space-y-6 text-white"><DashboardPageHeader title="Exam Results" description="Enter, review and analyze exam results and leaderboard performance." icon={ClipboardCheck}/>
  <DashboardStatGrid><DashboardStatCard title="Selected Exam" value={selectedExam?.title||'—'} subtitle={selectedExam?.status||'—'} icon={ClipboardCheck} color="blue"/><DashboardStatCard title="Results" value={String(results.length)} subtitle="Loaded result records" icon={ListChecks} color="green"/><DashboardStatCard title="Present" value={String(present)} subtitle="Students with marks" icon={Users} color="blue"/><DashboardStatCard title="Absent" value={String(absent)} subtitle="Absent records" icon={BarChart3} color="rose"/></DashboardStatGrid>
